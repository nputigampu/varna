...
router.get("/api/orders/:otherUserId", isLoggedIn, async(req, res) => {
    console.log("in get /api/friendship-status/:otherUserId ", req.params);
...
router.post("/api/orders", isLoggedIn, async(req, res) => {
    console.log("in post /api/orders ", req.body);
    ...
    await Promise.all(items.map(async(item) => {
        ...
        console.log("result from new order item", result, updResult);
    }));
    return res.json({
        neworder
    });
